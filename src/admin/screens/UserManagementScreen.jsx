...
export default function UserManagementScreen() {
  ...
  const handleResetPassword = (userId) => {
    console.log('Reset password sent to user:', userId)
  }
      ...
      >
        ...
        <div className="flex items-center gap-3 text-xs font-bold">
          <span className={`px-3 py-1.5 rounded-xl border ${isDark ? 'bg-slate-950 border-slate-800 text-slate-300' : 'bg-slate-50 border-slate-200 text-slate-700'}`}>
            Total Users: <strong className={isDark ? 'text-white' : 'text-slate-900'}>4,892</strong>
          </span>
      ...
      <div className={`border rounded-2xl overflow-hidden shadow-xs ${isDark ? 'bg-slate-900/90 border-slate-800' : 'bg-white border-slate-200'}`}>
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className={`border-b font-bold uppercase tracking-wider ${isDark ? 'bg-slate-950 border-slate-800 text-slate-400' : 'bg-slate-50 border-slate-200 text-slate-600'}`}>
                <th className="py-3.5 px-4">User Avatar / Name</th>
            ...
            <tbody className={`divide-y ${isDark ? 'divide-slate-800' : 'divide-slate-100'}`}>
              {paginatedUsers.map((user) => (
                <tr key={user.id} className={`transition-colors ${isDark ? 'hover:bg-slate-800/40' : 'hover:bg-slate-50'}`}>
                  <td className="py-3.5 px-4">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <span className={`font-bold block ${isDark ? 'text-slate-200' : 'text-slate-900'}`}>{user.name}</span>
                        <span className={`text-[11px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{user.email}</span>
                      </div>
                    </div>
                  </td>
                  <td className={`py-3.5 px-4 font-semibold ${isDark ? 'text-slate-300' : 'text-slate-800'}`}>{user.university}</td>
                  <td className="py-3.5 px-4">
                    <span
                  ...
                  <td className="py-3.5 px-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`font-bold ${isDark ? 'text-slate-200' : 'text-slate-900'}`}>{user.trustScore}</span>
                    </div>
                  </td>
                  <td className="py-3.5 px-4">
                    <span
                  ...
                  <td className={`py-3.5 px-4 font-mono font-medium ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{user.joinedDate}</td>
                  <td className="py-3.5 px-4 text-right">
                    ...
                    >
                      <SlidersHorizontal className="w-3.5 h-3.5" /> Quick Edit
                    </button>
                  </td>
                </tr>
              ))}
        ...
        <div className={`p-4 border-t flex items-center justify-between text-xs ${isDark ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
          <span className={isDark ? 'text-slate-400' : 'text-slate-600'}>
            Showing Page <strong className={isDark ? 'text-white' : 'text-slate-900'}>{currentPage}</strong> of <strong className={isDark ? 'text-white' : 'text-slate-900'}>{totalPages}</strong> ({filteredUsers.length} total users)
          </span>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`p-2 border rounded-lg disabled:opacity-40 disabled:cursor-not-allowed transition-colors ${
                isDark ? 'bg-slate-900 border-slate-800 text-slate-300 hover:bg-slate-800' : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-100 shadow-xs'
              }`}
            >
