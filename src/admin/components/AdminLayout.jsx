  ...
  return (
    ...
    >
      <div className="flex h-screen overflow-hidden relative">
        ...
        <aside
          className={`fixed md:static inset-y-0 left-0 z-50 flex flex-col border-r transition-all duration-300 relative ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'
            } ${isSidebarCollapsed ? 'w-20' : 'w-64'} ${isMobileMenuOpen ? 'translate-IconX-0' : '-translate-IconX-full md:translate-IconX-0'
            }`}
        >
          {/* Logo & Brand Header */}
          <div
            className={`h-16 flex items-center border-b relative transition-all ${isDark ? 'border-slate-800' : 'border-slate-100'
              } ${isSidebarCollapsed ? 'justify-center px-2' : 'justify-between px-4'}`}
          >
            <Link
              to="/admin"
              className={`flex items-center gap-3 overflow-hidden min-w-0 ${isSidebarCollapsed ? 'justify-center' : ''
                }`}
            ...
            >
              ...
              {!isSidebarCollapsed && (
                <div className="truncate min-w-0">
                  <span
                    className={`font-extrabold text-sm tracking-tight block truncate ${isDark ? 'text-white' : 'text-slate-900'
                      }`}
                  >
                    CampusHustle
                  </span>
            ...
            {!isSidebarCollapsed ? (
              <button
                ...
                className={`hidden md:flex p-1.5 rounded-lg transition-colors shrink-0 ${isDark
            ...
            <button
              onClick={() => setIsMobileMenuOpen(false)}
              className={`md:hidden p-1.5 rounded-lg ${isDark ? 'text-slate-400 hover:text-white' : 'text-slate-600 hover:text-slate-900'
                }`}
            >
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  onClick={() => setIsMobileMenuOpen(false)}
                  className={`flex items-center gap-3 px-3 py-3 rounded-xl text-xs font-semibold transition-all relative group ${active
          ...
          <div className={`p-3 border-t ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>
            <Link
              to="/market"
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all ${isDark
                  ? 'text-slate-400 hover:text-white hover:bg-slate-800'
                  : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
                } ${isSidebarCollapsed ? 'justify-center px-0' : ''}`}
              title={isSidebarCollapsed ? 'Marketplace View' : undefined}
            >
              <IconArrowRight className="w-4 h-4 text-emerald-600 shrink-0" />
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <header
            className={`h-16 flex items-center justify-between px-4 sm:px-8 border-b z-20 backdrop-blur-md transition-colors ${isDark ? 'bg-slate-900/80 border-slate-800' : 'bg-white/90 border-slate-200 shadow-xs'
              }`}
          >
            {/* Mobile IconMenu2 toggle & Global IconSearch */}
            <div className="flex items-center gap-3 flex-1 max-w-xl">
              <button
                onClick={() => setIsMobileMenuOpen(true)}
                className={`md:hidden p-2 rounded-xl border ${isDark
              ...
              <div className="relative w-full">
                <IconSearch
                  className={`w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 ${isDark ? 'text-slate-500' : 'text-slate-400'
                    }`}
                />
                <input
                  type="text"
                  ...
                  placeholder="Global IconSearch student IDs, reports, listings, or campuses..."
                  className={`w-full pl-10 pr-12 py-2 text-xs rounded-xl border outline-none transition-all ${isDark
                ...
                />
                <kbd
                  className={`absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-mono px-1.5 py-0.5 rounded-md border ${isDark
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <button
                onClick={toggleTheme}
                className={`p-2 rounded-xl transition-all border cursor-pointer ${isDark
              ...
              >
                {isDark ? <IconSun className="w-5 h-5" /> : <IconMoon className="w-5 h-5 text-indigo-600" />}
              </button>
              ...
              <div className="relative" ref={notificationRef}>
                <button
                  ...
                  className={`p-2 rounded-xl transition-colors relative border cursor-pointer ${isDark
                ...
                {showNotifications && (
                  <div
                    className={`absolute right-0 mt-3 w-80 rounded-2xl shadow-2xl p-4 z-50 border animate-in fade-in duration-150 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
                      }`}
                  >
                    <div
                      className={`flex items-center justify-between border-b pb-3 mb-3 ${isDark ? 'border-slate-800' : 'border-slate-100'
                        }`}
                    >
                      <h4
                        className={`text-xs font-bold flex items-center gap-1.5 ${isDark ? 'text-slate-100' : 'text-slate-900'
                          }`}
                      >
                        <IconBell className="w-4 h-4 text-indigo-600" /> Admin Notifications
                      </h4>
                    ...
                    <div className="space-y-2.5">
                      {mockNotifications.map((n) => (
                        <div
                          key={n.id}
                          className={`p-2.5 rounded-xl border text-xs ${isDark ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'
                            }`}
                        >
                          <p
                            className={`font-bold flex items-center justify-between ${isDark ? 'text-slate-200' : 'text-slate-800'
                              }`}
                          >
                            {n.title}
              ...
              <div className="relative" ref={profileMenuRef}>
                <button
                  ...
                  className={`flex items-center gap-2.5 pl-2 pr-3 py-1.5 rounded-xl transition-all border cursor-pointer ${isDark
                ...
                {showProfileMenu && (
                  <div
                    className={`absolute right-0 mt-3 w-56 rounded-2xl shadow-2xl p-2 z-50 border animate-in fade-in duration-150 text-xs ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
                      }`}
                  >
                    ...
                    <div className="py-1">
                      <Link
                        ...
                        className={`flex items-center gap-2 px-3 py-2 rounded-xl ${isDark ? 'text-slate-300 hover:bg-slate-800' : 'text-slate-700 hover:bg-slate-100'
                          }`}
                      >
                        Account Settings
                      </Link>
