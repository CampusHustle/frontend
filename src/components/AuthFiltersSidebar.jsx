...
export default function AuthFiltersSidebar() {
  return (
    <aside className="w-full md:w-64 flex-shrink-0 bg-white p-6 rounded-xl border border-gray-100 shadow-sm h-fit min-w-0">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-bold text-gray-900">Filters</h3>
        <button className="text-xs font-semibold text-orange-500 hover:text-orange-600">Clear All</button>
      </div>
        ...
        <div className="space-y-3">
          {['Computer Science', 'Economics', 'Mathematics'].map((category) => (
            <label key={category} className="flex items-center space-x-3 cursor-pointer min-w-0">
              <input 
                type="checkbox" 
                className="form-checkbox h-4 w-4 text-blue-900 rounded border-gray-300 focus:ring-blue-900" 
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-wrap gap-2">
          <button className="rounded-full bg-blue-950 px-4 py-1.5 text-xs font-medium text-white">All</button>
