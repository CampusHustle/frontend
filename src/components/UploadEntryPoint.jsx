...
export default function UploadEntryPoint({ onFileSelect }) {
  ...
  return (
    <div className="w-full max-w-md rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50 p-8 text-center">
      <div className="mb-6">
        <h3 className="text-lg font-bold text-yellow-900">Upload Study Materials</h3>
      ...
      <div className="flex flex-col gap-4 sm:flex-row justify-center">
        {/* PDF Upload Port */}
        <label
          className={`flex cursor-pointer items-center justify-center gap-2 rounded-lg bg-yellow-900 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-yellow-800 shadow-sm ${
        ...
        {/* Camera Capture Port */}
        <label
          className={`flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-5 py-3 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-100 shadow-sm ${
            isLoading ? 'opacity-50 cursor-not-allowed pointer-events-none' : ''
          }`}
        >
