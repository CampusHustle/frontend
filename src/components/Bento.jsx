...
function CellShell({ className = '', children }) {
  return (
    <div
      className={`relative overflow-hidden rounded-3xl border border-surface-variant bg-surface-lowest shadow-level-1 transition-[border-color,transform,box-shadow] duration-300 hover:-translate-y-1 hover:border-outline-variant hover:shadow-level-2 ${className}`}
    >
      {children}
    </div>
  )
    ...
    <CellShell className="min-h-[280px] md:col-span-2 md:min-h-[320px]">
      ...
      <div className="relative flex h-full flex-col justify-end p-7 lg:p-9">
        <span className="flex items-center gap-1.5 text-[13px] font-medium text-white/90">
          <IconMapPin size={16} className="text-hustle-400" />
          Walking-distance search
        </span>
        <h3 className="mt-2 max-w-md font-display text-2xl font-semibold leading-tight tracking-tight text-white lg:text-3xl">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {['2 min walk', 'Campus quad', 'Active now'].map((tag) => (
            <span
              key={tag}
              className="rounded-full border border-white/20 bg-black/40 px-3 py-1 text-xs font-medium text-white backdrop-blur-sm"
            >
              {tag}
            </span>
          ))}
  ...
  return (
    <CellShell className="flex min-h-[280px] flex-col justify-between bg-hustle-500 p-7 md:min-h-[320px]">
      <div className="flex items-start justify-between">
        <span className="flex size-11 items-center justify-center rounded-2xl bg-black/10">
    ...
    <CellShell className="min-h-[280px] p-7 md:min-h-[320px]">
      ...
      <div className="relative flex h-full flex-col justify-between">
        <span className="flex items-center gap-1.5 text-[13px] font-medium text-on-surface-variant">
          <IconCircleCheckFilled size={16} className="text-hustle-600" />
          Campus verified
        </span>
        <div>
          ...
          <div className="mt-5 flex items-center gap-3">
            <div className="flex -space-x-2">
              {['AK', 'JT', 'MB'].map((initials, i) => (
                <span
                  key={initials}
                  className="flex size-8 items-center justify-center rounded-full border border-white text-[11px] font-semibold text-ink-contrast shadow-sm"
  ...
  return (
    <CellShell className="min-h-[280px] p-7 md:min-h-[320px]">
      <div className="flex h-full flex-col justify-between">
        <span className="flex items-center gap-1.5 text-[13px] font-medium text-on-surface-variant">
          <IconMessageCircle size={16} className="text-hustle-600" />
          Chat & escrow
        </span>
        <div className="space-y-2">
          <div className="ml-auto w-fit max-w-[80%] rounded-2xl rounded-br-md bg-primary px-4 py-2.5 text-[13px] leading-snug text-on-primary shadow-sm">
            Can you shoot the club fair on Saturday?
          </div>
          <div className="w-fit max-w-[80%] rounded-2xl rounded-bl-md bg-surface-container px-4 py-2.5 text-[13px] leading-snug text-on-surface">
...
function CellE() {
  return (
    <CellShell className="flex min-h-[280px] flex-col justify-between bg-surface-low p-7 md:min-h-[320px]">
      <div className="flex items-center gap-1">
        {Array.from({ length: 5 }).map((_, i) => (
          <IconStar
            key={i}
            size={20}
            className={i === 4 ? 'text-outline-variant' : 'text-hustle-500'}
            fill="currentColor"
          />
        ))}
      </div>
